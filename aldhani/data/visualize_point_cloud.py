#!/usr/bin/env python3
"""
Quick point cloud visualization script for Ouster lidar data.
Reads the output.txt file generated by run.py and creates an interactive 3D plot.

Based on Ouster SDK best practices for matplotlib visualization.
"""
...
def create_visualization(data, color_mode='height', point_size=0.5, max_range=None, subsample=None):
    """
    ...
    # Set up the figure - following Ouster SDK best practices
    ...
    if color_mode == 'height':
        ...
        # Color by Z value capped at 5m (similar to Ouster SDK example)
    ...
    ax.set_ylabel('Y [m]')
    ax.set_zlabel('Z [m]')
    ax.set_title('Ouster Lidar Point Cloud Visualization\n(First Frame)')
    
    # Set reasonable axis limits based on data
    # Following Ouster SDK example of setting symmetric limits
    data_range = max(
...
def main():
    parser = argparse.ArgumentParser(description='Visualize Ouster point cloud data')
    parser.add_argument('filename', nargs='?', default='output.txt',
                        help='Point cloud file to visualize (default: output.txt)')
    parser.add_argument('--color', choices=['height', 'distance', 'intensity', 'uniform'],
                        default='height', help='Coloring mode (default: height)')
    parser.add_argument('--size', type=float, default=0.5,
                        help='Point size (default: 0.5)')
    parser.add_argument('--max-range', type=float, 
                        help='Maximum range to display in meters')
