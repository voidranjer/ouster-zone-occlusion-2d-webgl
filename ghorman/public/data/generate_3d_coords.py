#!/usr/bin/env python3
"""
Generate N random 3D coordinates scattered away from the z-axis.
...
Z coordinates range from -1 (ground level) to 1 units height.
...
def generate_random_3d_point_in_cylinder(
    min_cylinder_radius=1.0, max_cylinder_radius=2.0, min_z=-1.0, max_z=1.0
):
    """
    ...
    Args:
        ...
        min_z: Minimum z-coordinate (ground level, default: -1.0)
...
def main():
    ...
    print("Minimum distance from z-axis (cylindrical): 1.0 unit")
    print("Maximum distance from z-axis (cylindrical): 2.0 units")
    print("Z-coordinate range: -1.0 (ground) to 1.0 units (height)")
    ...
    print(f"  Z within range [-1, 1]: {-1.0 <= min_z and max_z <= 1.0}")
